...
![](docs/fig/representation_learning.svg){alt="Diagram showing raw inputs transformed by an encoder into learned representations, trained through supervised or self-supervised objectives."}
...
![](docs/fig/token_sentence_embeddings.svg){alt="Diagram showing a sentence tokenised, mapped into token vectors by an encoder, then combined into one sentence embedding for a downstream task."}
...
![](docs/fig/embedding_space_meaning.svg){alt="Schematic embedding space showing biology and space related texts forming separate regions, with a new example placed near its semantically similar neighbours."}
